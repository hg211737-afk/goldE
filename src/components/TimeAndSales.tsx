...
export const TimeAndSales: React.FC<TimeAndSalesProps> = ({ trades }) => {
  ...
  const displayedTrades = whalesOnly ? trades.filter((t) => t.isWhale) : trades;
    ...
    <div className="flex flex-col h-full bg-[#111622] rounded-xl border border-slate-800/80 overflow-hidden select-none">
      <div className="px-3 py-2 bg-slate-900/90 border-b border-slate-800 flex items-center justify-between">
        ...
        <button
          ...
          className={`flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-medium transition-all cursor-pointer ${
            whalesOnly ? "bg-amber-500 text-slate-950 font-bold shadow-xs" : "bg-slate-800 text-slate-400 hover:text-white"
          }`}
      ...
      <div className="flex-1 overflow-y-auto font-['JetBrains_Mono'] text-xs divide-y divide-slate-800/30">
        {displayedTrades.length === 0 ? (
          ...
          displayedTrades.slice(0, 30).map((trade) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-slate-200 flex items-center justify-center gap-1">
                  {trade.qty.toFixed(2)}
                  {trade.isWhale && (
                    <span className="text-[9px] bg-amber-400/20 text-amber-300 px-1 rounded">حوت</span>
                  )}
                </span>
